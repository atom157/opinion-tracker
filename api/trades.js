export default async function handler(req, res) {
  ...
  // Upstream priority:
  // 1) opinion-trade.space (no API key)
  // 2) openapi.opinion.trade (apikey fallback)
  const upstreams = [
    {
      name: 'opinion-trade.space',
      url: `https://www.opinion-trade.space/api/trades?address=${encodeURIComponent(safeAddress)}&limit=${encodeURIComponent(safeLimit)}`,
      headers: { 'Accept': 'application/json' }
    },
    {
      name: 'openapi.opinion.trade',
      url: `https://openapi.opinion.trade/openapi/trade/user/${encodeURIComponent(safeAddress)}?limit=${encodeURIComponent(safeLimit)}`,
